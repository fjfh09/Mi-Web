...
if (empty($name) || empty($email_raw) || empty($phone) || empty($message)) {
...
// El resto del código ya tiene el entorno cargado

// Variables SMTP - usando gemma@almagara.es
$smtp_host = $_ENV['SMTP_HOST'] ?? 'mail.almagara.es';
$smtp_port = $_ENV['SMTP_PORT'] ?? 465;
$smtp_user = 'gemma@almagara.es'; // Usuario configurado como requerido
$smtp_pass = $_ENV['SMTP_PASS_GEMMA'] ?? $_ENV['SMTP_PASS'] ?? ''; // Prioriza una contraseña específica para Gemma si existe, sino usa la general
...
try {
    $subject = "Mensaje Web Clases Gemma: $name";
    
    // Versión en texto plano para clientes sin soporte HTML y mejor puntuación antispam
    $text_message = "Mensaje recibido a través de la web:\n\n"
                  . "Nombre: $name\n"
                  . "Correo: $email\n"
                  . "Teléfono: $phone\n\n"
                  . "Mensaje:\n$message\n\n"
                  ...
                  . "© 2026 Gemma Clases. Todos los derechos reservados. https://clasesgemma.almagara.es\n";
    ...
    <head>
      ...
      <style>
        ...
        @media only screen and (max-width: 600px) {
          .container-table { border-radius: 8px !important; margin: 0 !important; }
          .header-td { padding: 30px 20px !important; }
          .header-title { font-size: 20px !important; }
          .content-td { padding: 25px 16px !important; }
          .message-box { font-size: 16px !important; padding: 16px !important; }
        }
        @media (prefers-color-scheme: dark) {
          body { background-color: #0f172a !important; }
          .outer-table { background-color: #0f172a !important; }
          .container-table { background-color: #1e293b !important; border-color: #334155 !important; }
          .content-td { color: #e2e8f0 !important; }
          .message-box { background-color: #0f172a !important; color: #f1f5f9 !important; border-left-color: #818cf8 !important; }
          .detail-label { color: #94a3b8 !important; border-bottom-color: #334155 !important; }
          .detail-value { color: #f1f5f9 !important; border-bottom-color: #334155 !important; }
          .footer-text { color: #94a3b8 !important; }
          .footer-sep { border-top-color: #334155 !important; }
        }
      </style>
    </head>
              ...
              <tr>
                <td class='content-td' style='padding: 35px 24px;'>
                  <p style='margin: 0 0 24px 0; font-size: 16px; color: #4b5563; line-height: 1.6;'>
                    Hola Gemma, has recibido una nueva solicitud a través del formulario de contacto de tu web. Estos son los detalles del alumno:
                  </p>
                  <table width='100%' cellpadding='0' cellspacing='0' border='0' style='font-size: 15px; border-collapse: collapse; text-align: left; margin-bottom: 30px;'>
                    ...
                    <tr>
                      ...
                      <td class='detail-value' style='padding: 14px 0; color: #0f172a; font-weight: 600; border-bottom: 1px solid #e2e8f0;'>
                        <a href='https://wa.me/34" . preg_replace('/[^0-9]/', '', $phone) . "' style='color: #25D366; text-decoration: none;'>$phone (Chat WA)</a>
                      </td>
                    </tr>
                  </table>
                        ...
                        <table cellpadding='0' cellspacing='0' border='0' style='margin: 0 auto;'>
                          <tr>
                            <td align='center' bgcolor='#25D366' style='border-radius: 8px;'>
                              <a href='https://wa.me/34" . preg_replace('/[^0-9]/', '', $phone) . "' target='_blank' style='font-size: 15px; font-family: -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, Helvetica, Arial, sans-serif; color: #ffffff; text-decoration: none; border-radius: 8px; padding: 14px 28px; border: 1px solid #25D366; display: inline-block; font-weight: 600;'>
            ...
            <table width='100%' cellpadding='0' cellspacing='0' border='0' style='max-width: 600px; margin: 0 auto; margin-top: 24px;'>
              <tr>
                <td align='center' class='footer-text' style='padding: 10px 20px; font-size: 12px; color: #6b7280; line-height: 1.6;'>
                  <p style='margin: 0 0 16px 0;'>Este es un correo automático generado por la web de tus clases particulares.</p>
                  <table cellpadding='0' cellspacing='0' border='0' style='margin: 0 auto;'>
                    <tr>
                      <td align='center' class='footer-sep' style='border-top: 1px solid #e5e7eb; padding-top: 16px;'>
                        <p style='margin: 0; font-size: 13px; color: #4b5563; font-weight: 500;'>
                          <strong>© 2026 Clases Gemma.</strong> Todos los derechos reservados.
                        </p>
                        <p style='margin: 4px 0 0 0; font-size: 12px;'>
                          <a href='https://clasesgemma.almagara.es' target='_blank' style='color: #6366f1; text-decoration: none; font-weight: 600;'>clasesgemma.almagara.es</a>
                        </p>
    ...
    // Destinatarios a los que se enviará la copia (leídos desde .env para no filtrar los correos personales)
    $mail_gemma = $_ENV['MAIL_TO_GEMMA'] ?? 'info@tudominio.com';
    $mail_almagara = $_ENV['MAIL_TO_ALMAGARA'] ?? 'info@tudominio.com';
    $mail_javier = $_ENV['MAIL_TO_JAVIER'] ?? 'info@tudominio.com';
    $recipients = [$mail_gemma, $mail_almagara, $mail_javier];
    ...
    if (!$socket) throw new Exception("Error SMTP: $errstr");
    ...
    function smtp_read($socket, $expected) {
        ...
        if (substr($response, 0, 3) != $expected) throw new Exception("SMTP Error: $response");
        return $response;
    }
    ...
    foreach ($recipients as $recipient) {
        ...
        $headers = [
            ...
            "From: Contacto Clases Gemma <$smtp_user>",
            "Reply-To: $name <$email>",
            "To: <$recipient>",
        ...
        $body = implode("\r\n", $headers) . "\r\n\r\n" . $multipart_body . "\r\n.\r\n";
        fwrite($socket, $body); smtp_read($socket, 250);
    }
